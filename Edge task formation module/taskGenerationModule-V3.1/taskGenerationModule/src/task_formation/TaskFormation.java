...
public class TaskFormation {
    ...
    // claculate standard deviation of execution time of microservices
    private void calculateStandardDeviation() {
        ...
        for(MicroService microServiceInstance: microServiceList) {
            sumExecutionTime += Math.pow(microServiceInstance.getMicroServiceExecutionTime() - avgExecutionTime, 2.0);
        }
        standardDeviation = Math.sqrt(sumExecutionTime / noOfMicroService);
    }
    ...
    // method for calculating average execution time of microservices within a task
    private double calculateAvgExecutionTimeOfTask(ArrayList<MicroService> microServiceList, double newMicroServiceTime) {
        double totalExecutionTime = 0;
        int n = microServiceList.size();
        for(MicroService microServiceInstance: microServiceList) {
            totalExecutionTime += microServiceInstance.getMicroServiceExecutionTime();
        }

        if(newMicroServiceTime == 0) {
            return (totalExecutionTime / n);
        }
        else {
            n++;
            totalExecutionTime += newMicroServiceTime;
            return (totalExecutionTime / n);
        }
    }
    ...
    private boolean checkExecutionTimeConstraint(int parentVertexIndex, int vertexIndex) {
        ...
        int parentVertexTaskId = parentMicroServiceInstance.getTaskGroupId();
        Task parentVertexTask = Task.getTask(parentVertexTaskId);
        ArrayList<MicroService> microServiceList = parentVertexTask.getMicroServiceListOfTask();
        int microServiceListSize = microServiceList.size();

        double currentAvgExecutionTime = calculateAvgExecutionTimeOfTask(microServiceList, 0);
        double newAvgExecutionTime = calculateAvgExecutionTimeOfTask(microServiceList, childMicroServiceInstance.getMicroServiceExecutionTime());

        /* Approach 1: total execution time < avg of all microservices */
        // if(childMicroServiceInstance.getMicroServiceExecutionTime() + parentVertexTask.getTaskExecutionTime() <= avgExecutionTime) {
        //     return true;
        // }

        /* Approach 2: total execution time < avg of all microservices + sd */
        if(childMicroServiceInstance.getMicroServiceExecutionTime() + parentVertexTask.getTaskExecutionTime() <= (avgExecutionTime + standardDeviation)) {
            return true;
        }

        /* Approach 3: new avg execution time < current avg execution time + sd */
        // if((newAvgExecutionTime >= (currentAvgExecutionTime - standardDeviation)) && (newAvgExecutionTime <= (currentAvgExecutionTime + standardDeviation))) {
        //     return true;
        // }

        /* Approach 4: new avg execution time < global avg execution time + sd */
        // if((newAvgExecutionTime <= (avgExecutionTime + standardDeviation)) && (newAvgExecutionTime >= (avgExecutionTime - standardDeviation))) {
        //     return true;
        // }
