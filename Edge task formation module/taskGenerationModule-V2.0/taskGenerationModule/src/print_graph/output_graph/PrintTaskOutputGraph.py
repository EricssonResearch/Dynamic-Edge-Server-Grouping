...
import ParseTaskOutputGraph

def drawGraph(colorList):

    edgeList = ParseTaskOutputGraph.parseXMLOutputFile()
    ...
    for edge in edgeList:
        vertexName = "t" + str(edge.getVertexId())
    ...
    for edges in edgeList:
        ...
        for pair in pairList:
            G.add_edge("t"+srcVertex, "t"+pair[0], weight=int(pair[1]))
    ...
    node_colors = [colorList[node] for node in G.nodes()]
    ...
    # Display the graph
    plt.title("Task Output Graph")
    plt.axis("off")
    plt.savefig('output/task_output_graph.jpg', dpi=300, bbox_inches='tight')
    plt.show()
    plt.clf()
